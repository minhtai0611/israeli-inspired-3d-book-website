# ADR 0003: Server-side filter/sort/pagination for `/library/[category]`

## Status

Accepted, implemented in `src/app/library/[category]/page.tsx`.

## Context

`/library/[category]` passed its *entire* category's book array to a client component
(`CategoryBrowser`) that did search/sort/pagination in the browser, showing 24 at a time. Measured
before the fix: `/library/Halakhah` (2,169 books in that category) shipped 555.9 KB of raw HTML —
...
`SearchForm.tsx` progressive-enhancement convention already used on `/search`) instead of a
...
- **Debounced client-side search hitting an API route.** Rejected as unnecessary complexity: a plain
  ...
  site has no live-typing-search requirement (the existing `/search` already established this
  pattern site-wide).

## Consequences

Measured: `/library/Halakhah` raw HTML dropped from 555.9 KB to 72.4 KB (compressed: 42.7 KB →
